...
namespace Project.Templates.Scripts.Camera.Effect
{
    public class Shaker : MonoBehaviour
    {
        ...
        public void Shake(float duration, float magnitude, float noise, Vector2 point, bool shake3D, float maxForce = 1)
        {
            ...
            _ = _shake3D
                ? StartCoroutine(Shake3DCoroutine(duration, force * magnitude, force * noise))
                : StartCoroutine(Shake2DCoroutine(duration, force * magnitude, force * noise));
        }

        private IEnumerator Shake2DCoroutine(float duration, float magnitude, float noise)
        {
            float elapsed = 0f;
        ...
        private IEnumerator Shake3DCoroutine(float duration, float magnitude, float noise)
        {
            float elapsed = 0f;
            Vector2 noiseStartPointX = Random.insideUnitCircle * noise;
            Vector2 noiseStartPointY = Random.insideUnitCircle * noise;
